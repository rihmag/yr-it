...
export default function Artical() {
  ...
  const articles = [
    ...
    {
      id: "job-ready-skills",
      title: "Job‑Ready in 90 Days: A Practical Plan for Beginners",
      excerpt:
        "A realistic weekly plan combining courses, projects, and public proof to reach interviews faster.",
      category: "Career",
      readTime: 11,
      date: "2025-04-15",
      author: "Aman Gupta",
      tags: ["Career", "Planning", "Projects"],
      cover: "/images/articles/job-ready.jpg",
    },
  ];
...
function ArticleCard({ article }) {
  return (
    <div className="group relative bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all">
      {/* Cover */}
      <div className="aspect-[16/9] bg-gradient-to-br from-blue-200 to-purple-200 dark:from-blue-900/30 dark:to-purple-900/30 overflow-hidden">
        {/* Optional image slot */}
        {/* <img src={article.cover} alt={article.title} className="w-full h-full object-cover" /> */}
        <div className="absolute top-3 left-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-white/80 dark:bg-gray-900/70 border border-gray-200 dark:border-gray-700 text-blue-700 dark:text-blue-300">
          <Tag size={12} />
          {article.category}
        </div>
      </div>
      ...
      {/* Body */}
      <div className="p-5">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 line-clamp-2">
          {article.title}
        </h3>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
          <span className="inline-flex items-center gap-1"><User size={14} /> {article.author}</span>
          <span className="inline-flex items-center gap-1"><Clock size={14} /> {article.readTime} min read</span>
          <span className="inline-flex items-center gap-1"><CalendarIcon size={14} /> {new Date(article.date).toLocaleDateString()}</span>
        </div>

        {/* Tags */}
        <div className="mt-3 flex flex-wrap gap-2">
          {article.tags.map((t) => (
            <span key={t} className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-800">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          <button className="p-2 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-300 transition-colors">
            <Bookmark size={16} />
          </button>
        </div>
      </div>
