...
const usePicker = ({
  ...
  onSuccess: _onSuccess = console.log,
  onUploadDone: _onUploadDone2 = console.log,
  onError: _onError2 = console.error
}) => {
  ...
  const _onUploadDone = result => {
    _onSuccess(result);
    _onUploadDone2(result);
  };
  const rootId = _generateRandomId();
  const containerId = _generateRandomId();
  useEffect(() => {
    const picker = filestack.Filestack(apikey, _clientOptions).picker(_extends({
      rootId,
  ...
  }, [rootId, containerId, apikey, _clientOptions, _pickerOptions, _onUploadDone]);
  return {
    containerId
  };
};
...
//# sourceMappingURL=filestack-react.modern.mjs.map
